...
export const TextAssetPanel: React.FC = () => {
  const { text, importTextCatalogs, importTextTimeline, addTextCue, injectCatalogEntry, setPreviewLocale, playhead } = useProject();
  ...
  const activeLocale: LocaleCode | null =
    text.previewLocale ?? (sortedLocales.includes('en') ? 'en' : (sortedLocales[0] as LocaleCode | undefined) ?? null);
    ...
    <div className="text-asset-panel">
      ...
      <div className="panel-section">
        ...
        <p className="asset-panel-hint" style={{ fontSize: '11px', color: 'var(--color-text-secondary)', marginBottom: '14px', lineHeight: '1.4', background: 'rgba(255,255,255,0.03)', padding: '8px 12px', borderRadius: '6px', border: '1px solid rgba(255,255,255,0.05)' }}>
          💡 Type text to add it to your project, then place it on the timeline to show it in the video.
        </p>

        <form onSubmit={handleAddText} style={{ display: 'flex', gap: '8px', marginBottom: '10px' }}>
          <input
            type="text"
        ...
        <div className="panel-section" style={{ marginTop: '20px' }}>
          ...
          <div className="locale-list">
            {sortedLocales.map(locale => {
              ...
              const isPreview = text.previewLocale === locale || (text.previewLocale === null && locale === activeLocale);
                  ...
                  <div key={key} className="key-item" style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '8px' }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: '12px', fontWeight: 500, color: 'var(--color-text-primary)', whiteSpace: 'pre-wrap', wordBreak: 'break-word' }}>{previewValue}</div>
                      </div>
                      <button
                        className="btn btn-primary btn-sm"
      ...
      <div className="panel-section" style={{ marginTop: '20px' }}>
        ...
        <p className="asset-panel-hint" style={{ fontSize: '11px', color: 'var(--color-text-secondary)', marginBottom: '14px', lineHeight: '1.4' }}>
          Optional: import locale catalogs (en.json, sv.json) + timeline.json, or export your catalogs to translate them offline.
        </p>
