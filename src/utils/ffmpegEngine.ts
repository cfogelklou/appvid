...
export async function processVideo(
  ...
  try {
    ...
    const uniqueAssetIds = Array.from(
      new Set(project.segments.map((s) => s.assetId)),
    );
    ...
    for (const assetId of uniqueAssetIds) {
      ...
      logCallback(
        `Writing audio asset to virtual filesystem: ${audioFilename}`,
      );
    ...
    if (segments.length === 1) {
      ...
      videoFilters.push(
        `${concatLabels.join("")}concat=n=${segments.length}:v=1:a=0[trimmed_v]`,
      );
      videoSourceLabel = "[trimmed_v]";
    }
    ...
    if (keepOriginalAudio && hasAudio) {
      ...
      if (segments.length === 1) {
        ...
        audioFilters.push(
          `${concatLabels.join("")}concat=n=${segments.length}:v=0:a=1[trimmed_a]`,
        );
        origAudioLabel = "[trimmed_a]";
      } else {
        origAudioLabel = "[0:a]";
    ...
    project.segments.forEach((segment, idx) => {
      ...
      if (filename) {
        ...
        const duration =
          segment.duration !== undefined ? segment.duration : assetDuration;
        const clipStart =
          segment.clipStart !== undefined ? segment.clipStart : 0;
    ...
    const uint8Data =
      outData instanceof Uint8Array ? outData : new Uint8Array();
    const outBlob = new Blob([uint8Data.buffer as ArrayBuffer], {
      type: "video/mp4",
    });
...
export async function renderVideo(
  ...
  try {
    ...
    logCallback(
      `Writing ${textOverlays.length} text overlays to virtual filesystem`,
    );
    ...
    logCallback(
      `Executing FFmpeg command with text overlays: ffmpeg ${execArgs.join(" ")}`,
    );
